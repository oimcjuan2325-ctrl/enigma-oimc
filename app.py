...
if st.session_state.usuario is None:
    ...
    with tabs[2]:
        ...
        for m in [m for m in db["mensajes"] if m["a"] == "CHAT GRUPAL"]:
            st.caption(f"{m['de']} ({m['fecha']})")
            st.code(m['msg'])
        msg_g = st.text_area("Mensaje al grupo:", height=100)
        if st.button("Enviar al grupo"):
            db["mensajes"].append({"de": u, "a": "CHAT GRUPAL", "msg": traducir(msg_g, "cifrar"), "fecha": datetime.now().strftime("%d/%m/%Y %H:%M"), "leido": False})
    ...
    with tabs[3]:
        ...
        if st.button("Enviar mensaje privado"):
            db["mensajes"].append({"de": u, "a": dest, "msg": traducir(msg_i, "cifrar"), "fecha": datetime.now().strftime("%d/%m/%Y %H:%M"), "leido": False})
            guardar_db(db); st.rerun()
